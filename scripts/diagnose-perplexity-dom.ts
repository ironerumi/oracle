...
import { launchCamoufox, cdpCookiesToPlaywright } from '../src/perplexity-browser/camoufoxLifecycle.js';
...
async function main() {
  ...
  try {
    ...
    try {
      ...
      const authOnly = raw.filter((c: any) =>
    ...
    const diag = await page.evaluate(() => {
      ...
      results['isCloudflare'] = document.title.toLowerCase().includes('just a moment') ||
        document.title.includes('しばらくお待ちください');

      // Check for login state
      const loginTexts = ['Continue with Google', 'Googleで続ける'];
      results['hasLoginPrompt'] = loginTexts.some(t =>
