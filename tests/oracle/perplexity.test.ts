...
describe("buildMessages", () => {
  ...
  test("handles multiple inputs", () => {
    ...
    } as unknown as OracleRequestBody;
...
describe("parsePerplexityResponse", () => {
  ...
  test("falls back to status code on malformed error JSON", async () => {
    ...
    await expect(parsePerplexityResponse(mockResponse)).rejects.toThrow(
  ...
  test("handles empty choices array", async () => {
    const mockResponse = new Response(JSON.stringify({ id: "test", choices: [], usage: {} }), {
      status: 200,
    });
