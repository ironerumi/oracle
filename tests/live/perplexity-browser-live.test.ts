...
(live ? describe : describe.skip)("Perplexity browser live smoke", () => {
  ...
  it("basic query returns response with citations", async () => {
  ...
  it("space query routes to correct space", async () => {
  ...
  it("expired/missing cookies produces clear error", async () => {
    const config = makeBrowserConfig({
      inlineCookies: [
        { name: "bogus", value: "expired", domain: ".perplexity.ai" },
      ] as BrowserSessionConfig["inlineCookies"],
    });
    const execute = createPerplexityBrowserExecutor(config);
    await expect(
      execute({
  ...
  it("rejects attachments", async () => {
    ...
    await expect(
      execute({
        prompt: "test",
        attachments: [{ path: "/tmp/test.txt", displayPath: "test.txt" }],
        log: () => {},
      }),
