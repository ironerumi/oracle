...
export async function submitPerplexityPrompt(
  ...
  await page.evaluate((args: { selectors: string[]; text: string }) => {
  ...
  const submitResult = await page.evaluate((selectors: string[]) => {
    ...
    // Diagnostic info
    const editorText = (document.querySelector('[data-lexical-editor]') as HTMLElement)?.innerText?.slice(0, 100) ?? '(no editor)';
  ...
  if (!submitResult?.clicked) {
    const diag = submitResult as { editorText?: string; buttons?: Array<{ aria: string | null; disabled: boolean; text: string; hasSvg: boolean }> };
