...
async function waitForCompletion(
  ...
  while (Date.now() < deadline) {
    const result = await page.evaluate(
      (args: { copySelectors: string[]; proseSelector: string }) => {
        ...
        const hasProse = !!(
          document.querySelector(args.proseSelector) as HTMLElement
        )?.innerText?.trim();
        if (hasProse) {
    ...
    if (result?.done) {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        const curLen = await page.evaluate(
          (sel: string) => (document.querySelector(sel) as HTMLElement)?.innerText?.length ?? 0,
          RESPONSE_PROSE_SELECTOR,
        );
